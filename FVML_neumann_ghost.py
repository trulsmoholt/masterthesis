...
def compute_vector(mesh,vector,f,boundary=None):
    nodes = mesh.nodes
    cell_centers = mesh.cell_centers
    num_unknowns = cell_centers.shape[1]*cell_centers.shape[0]
    nx = nodes.shape[1]
    ...
    for i in range(cell_centers.shape[0]):
        for j in range(cell_centers.shape[1]):
            if (i==ny-2) or (j==0) or (j==nx-2):
                #vector[meshToVec(i,j)]= 0#boundary(cell_centers[i,j,0],cell_centers[i,j,1])
                continue
            if i==0:
...
if __name__=='__main__':
    ...
    y = sym.Symbol('y')
    K = np.array([[1,0],[0,1]])
    u_fabric = sym.cos(x*math.pi)*sym.sin(y*math.pi)
    f = -divergence(gradient(u_fabric,[x,y]),[x,y])
    
    mesh = Mesh(5,5,lambda p:np.array([p[0],p[1]]),ghostboundary=True)
    u_fabric = sym.lambdify([x,y],u_fabric)
    f = sym.lambdify([x,y],f)
    
    permeability = np.ones((mesh.cell_centers.shape[0],mesh.cell_centers.shape[1]))
    for j in range(mesh.cell_centers.shape[1]):
        for i in range(mesh.cell_centers.shape[0]):
            if i==mesh.cell_centers.shape[1]-1 or j==0 or j==mesh.cell_centers.shape[0]-1:
                permeability[i,j]=0
    ...
    B = np.zeros((mesh.num_unknowns,mesh.num_unknowns))
    B = mass_matrix(mesh)
    compute_matrix(mesh,A,K,permeability)

    b_indexes = []
    for i in range(mesh.cell_centers.shape[0]):
        for j in range(mesh.cell_centers.shape[1]):
            if  i==mesh.cell_centers.shape[0]-1 or j==0 or j==mesh.cell_centers.shape[1]-1:
                b_indexes.append(mesh.meshToVec(i,j))
    A = np.delete(A,b_indexes,0)
    A = np.delete(A,b_indexes,1)
    B = np.delete(B,b_indexes,0)
    B = np.delete(B,b_indexes,1)
    compute_vector(mesh,b,f,u_fabric)
    b = np.delete(b,b_indexes,0)
    u = np.linalg.solve(A,b)
    plotmesh = Mesh(6,5,lambda p:np.array([p[0],p[1]]))
    print(b.shape)
    plotmesh.plot_funtion(u_fabric,'u exact')
    plotmesh.plot_vector(b,'u approximation')
