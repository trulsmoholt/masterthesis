...
def compute_vector(mesh,vector,f,boundary,neumann_boundary):
    nodes = mesh.nodes
    cell_centers = mesh.cell_centers
    num_unknowns = cell_centers.shape[1]*cell_centers.shape[0]
    nx = nodes.shape[1]
    ...
    for i in range(cell_centers.shape[0]):
        for j in range(cell_centers.shape[1]):
            if (i==ny-2) or (i==0) or (j==nx-2):
                ...
                continue
            if j==0:
                vector[meshToVec(i,j)] -= neumann_boundary[i]
...
if __name__=='__main__':
    ...
    mesh = Mesh(6,6,lambda p: np.array([p[0] ,p[1]]),ghostboundary='neumann')
    mesh.plot()
